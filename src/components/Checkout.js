...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(userBalance<getTotalCartValue(items)){
      enqueueSnackbar('You do not have enough balance in you wallet for this purchase',
      {variant:'error'}
      );
      return false;
    }
    if(addresses.all.length<=0){
      enqueueSnackbar('Please add a new address before procedding',{
        variant:'warning'
      });
      return false;
    }
  ...
  useEffect(()=>{
    if(token) {
      const allAddressData=getAddresses(token);
    }
    else{
              ...
              
              {
                addresses.all.length ? (
                  addresses.all.map((item)=>(
                    ...
                    key={item._id}
                    className={
                      addresses.selected===item._id?'addresses-item selected':'addresses-item not-selected'
                    }
                    onClick={async()=>{
